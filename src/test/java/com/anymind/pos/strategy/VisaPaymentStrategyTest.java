...
class VisaPaymentStrategyTest {
    ...
    @InjectMocks
    private VisaPaymentStrategy visaPaymentStrategy;

    private final static UUID paymentMethodUUID = UUID.randomUUID();

    @BeforeEach
    void setUp() {
        when(paymentMethodRepository.findByPaymentMethodType(PaymentMethodType.VISA)).thenReturn(
        ...
        visaPaymentStrategy.initializeModifiers();
    }
    ...
    void testProcessPayment_Successful() {
        ...
        PaymentResponseDTO response = visaPaymentStrategy.processPayment(request);
    ...
    void testProcessPayment_InvalidPriceModifier() {
        ...
        assertThrows(InvalidPaymentRequestException.class, () -> visaPaymentStrategy.processPayment(request));
    ...
    void testGetPaymentMethod() {
        PaymentMethodType paymentMethodType = visaPaymentStrategy.getPaymentMethod();

        assertEquals(PaymentMethodType.VISA, paymentMethodType);
    }
}
