...
class MastercardPaymentStrategyTest {
    ...
    @InjectMocks
    private MastercardPaymentStrategy mastercardPaymentStrategy;

    private final static UUID paymentMethodUUID = UUID.randomUUID();

    @BeforeEach
    void setUp() {
        when(paymentMethodRepository.findByPaymentMethodType(PaymentMethodType.MASTERCARD)).thenReturn(
        ...
        mastercardPaymentStrategy.initializeModifiers();
    }
    ...
    void testProcessPayment_Successful() {
        ...
        PaymentResponseDTO response = mastercardPaymentStrategy.processPayment(request);
    ...
    void testProcessPayment_InvalidPriceModifier() {
        ...
        assertThrows(InvalidPaymentRequestException.class, () -> mastercardPaymentStrategy.processPayment(request));
    ...
    void testGetPaymentMethod() {
        PaymentMethodType paymentMethodType = mastercardPaymentStrategy.getPaymentMethod();

        assertEquals(PaymentMethodType.MASTERCARD, paymentMethodType);
    }
}
