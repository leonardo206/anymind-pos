...
class GrabPayPaymentStrategyTest {
    ...
    @InjectMocks
    private GrabPayPaymentStrategy grabPayPaymentStrategy;

    private final static UUID paymentMethodUUID = UUID.randomUUID();

    @BeforeEach
    void setUp() {
        when(paymentMethodRepository.findByPaymentMethodType(PaymentMethodType.GRAB_PAY))
        ...
        grabPayPaymentStrategy.initializeModifiers();
    }
    ...
    void testProcessPaymentSuccess() {
        ...
        PaymentResponseDTO response = grabPayPaymentStrategy.processPayment(request);
    ...
    void testPriceModifierOutOfBounds() {
        ...
        InvalidPaymentRequestException exception = assertThrows(InvalidPaymentRequestException.class, () -> {
            grabPayPaymentStrategy.processPayment(request);
        });
